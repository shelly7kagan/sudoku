...
void TestSudokuBoard::test_get_status(){
    std::vector<std::vector<int>> win_values = std::vector({std::vector<int>({7, 8, 5, 9, 3, 1, 4, 6, 2}),
    ...
    win_test_board.empty_options(); // this is for checking without the update function - needs to be deleted!
    assert(win_test_board.get_status() == WIN);

    std::vector<std::vector<int>> loss_row_values = std::vector({std::vector<int>({7, 8, 5, 9, 3, 1, 4, 6, 2}),
                                                                std::vector<int>({2, 4, 3, 8, 5, 6, 9, 7, 1}),
                                                                std::vector<int>({6, 9, 1, 7, 2, 4, 5, 8, 3}),
                                                                std::vector<int>({4, 5, 7, 6, 1, 2, 3, 9, 8}),
                                                                std::vector<int>({9, 6, 8, 5, 7, 3, 2, 1, 4}),
